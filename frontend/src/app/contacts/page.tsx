...
export default function ContactsPage() {
	...
	const fetchContacts = async () => {
		...
		try {
			...
			if (typeFilter && typeFilter !== 'all')
				params.append('contact_type', typeFilter)

			const response = await fetch(
				`http://localhost:3001/api/v1/contacts/?${params}`,
				{
					headers: {
	...
	const updateContact = async () => {
		...
		try {
			const response = await fetch(
				`http://localhost:3001/api/v1/contacts/${editingContact.id}`,
				{
					method: 'PUT',
					headers: {
						'Content-Type': 'application/json',
		...
		<div className='grid grid-cols-2 gap-4'>
			...
			<div>
				...
				<Select
					...
					onValueChange={value =>
						setFormData({ ...formData, contact_type: value as any })
					}
				>
					<SelectTrigger>
						<SelectValue />
						...
						<div className='flex gap-4 items-end'>
							...
							>
								<DialogTrigger asChild>
									<Button onClick={() => resetForm()}>Создать контакт</Button>
								</DialogTrigger>
