  ...
  React.useEffect(() => {
    ...
    if (token && userData && userData !== 'undefined' && userData !== 'null') {
      try {
        ...
        // Если пользователь уже вошёл, перенаправляем на соответствующую страницу
        if (user.role === 'ADMIN') {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Декодируем JWT токен напрямую для получения данных пользователя
        try {
          ...
          const userData = {
            id: payload.sub, // ID пользователя из токена
            email: payload.email || loginData.email, // Email из токена или формы
            role: payload.role, // Роль из токена
          ...
          // Fallback - сохраняем минимальную информацию и идём на dashboard
          localStorage.setItem('user', JSON.stringify({ email: loginData.email }))
          router.push('/dashboard')
        }
      } else {
        const errorData = await response.json()
        setError(errorData.message || errorData.detail || 'Ошибка при входе')
      }
    } catch (err) {
    ...
    try {
      const response = await fetch('http://localhost:3002/api/v1/auth/register', {
        ...
        body: JSON.stringify({
          ...
          passwordConfirm: registerData.password_confirm,
      ...
      if (response.ok) {
        ...
        const errorData = await response.json()
        setError(errorData.message || errorData.detail || 'Ошибка при регистрации')
      }
    } catch (err) {
      setError('Ошибка подключения к серверу')
    } finally {
