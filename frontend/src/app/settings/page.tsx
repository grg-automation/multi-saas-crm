...
import { getToken, logout, redirectToLogin } from '@/lib/auth'
import {
	AlertTriangle,
	Bell,
	Globe,
...
export default function SettingsPage() {
	...
	const updateSettings = async () => {
		...
		try {
			const response = await fetch(
				'http://localhost:3001/api/v1/users/settings',
				{
					method: 'PUT',
					headers: {
	...
	const deleteAccount = async () => {
		const confirmation = prompt(
			'Для подтверждения удаления аккаунта введите "DELETE":'
		)
		...
		try {
			const response = await fetch(
				'http://localhost:3001/api/v1/users/delete',
				{
					method: 'DELETE',
					headers: {
						Authorization: `Bearer ${token}`,
					...
					<div className='flex items-center justify-between'>
						<div>
							...
							<p className='text-gray-600 mt-2'>
								Управление предпочтениями и безопасностью аккаунта
							</p>
						</div>
						...
						<CardContent className='space-y-4'>
							<div className='flex items-center justify-between'>
								<div>
									<Label>Пароль</Label>
									<p className='text-sm text-gray-600'>
										Последнее изменение: более месяца назад
									</p>
								</div>
								<Dialog
